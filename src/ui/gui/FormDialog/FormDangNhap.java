...
public class FormDangNhap extends JFrame {
    ...
    public FormDangNhap() {
        ...
        // Load logo if exists
        try {
            // Make sure "img/logo.png" is accessible relative to the compiled classes
            java.net.URL logoUrl = getClass().getClassLoader().getResource("img/logo.png");
            if (logoUrl != null) {
        ...
        btnDangNhap.addMouseListener(new MouseAdapter() {
            ...
            @Override public void mousePressed(MouseEvent e) { btnDangNhap.setBackground(darkerColor); }
            @Override public void mouseReleased(MouseEvent e) { btnDangNhap.setBackground(originalColor); }
            @Override public void mouseEntered(MouseEvent e) { btnDangNhap.setBackground(darkerColor); }
            @Override public void mouseExited(MouseEvent e) { btnDangNhap.setBackground(originalColor); }
        });
    ...
    // --- (styleField, setPlaceholder, togglePasswordVisibility methods remain the same) ---
    ...
    private void setPlaceholder(JTextComponent comp, String text) {
        ...
        comp.addFocusListener(new FocusAdapter() {
            @Override public void focusGained(FocusEvent e) {
                ...
                String currentText = (comp instanceof JPasswordField) ? String.valueOf(((JPasswordField) comp).getPassword()) : comp.getText();
                if(currentText.equalsIgnoreCase(text) && comp.getForeground().equals(new Color(150,150,160))) { // Check if it's placeholder
                    comp.setText("");
    ...
    private void onLogin() {
        ...
        if (user.isEmpty() || user.equalsIgnoreCase(PLACEHOLDER_USER)) {
            JOptionPane.showMessageDialog(this, "Vui lòng nhập mã nhân viên.", "Thiếu thông tin", JOptionPane.WARNING_MESSAGE);
            txtTenDangNhap.requestFocusInWindow();
            return;
        }
        // Check password against placeholder AND empty string
        if (pass.isEmpty() || (String.valueOf(txtMatKhau.getPassword()).equals(PLACEHOLDER_PASS) && txtMatKhau.getForeground().equals(new Color(150,150,160)))) {
        ...
        try {
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Lỗi khi kiểm tra đăng nhập: " + ex.getMessage(), "Lỗi CSDL", JOptionPane.ERROR_MESSAGE);
        } catch (Exception ex){
            ex.printStackTrace();
    ...
    static class BackgroundPanel extends JPanel {
        @Override protected void paintComponent(Graphics g) {
            ...
            g2.setColor(new Color(200,230,255,80)); g2.fillOval(w-360, 40, 300, 300);
            g2.setColor(new Color(255,220,235,60)); g2.fillOval(40, h-360, 300, 300);
            g2.dispose();
        }
    }
    static class CardPanel extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            g2.setColor(new Color(0,0,0,30)); g2.fillRoundRect(6, 8, w-12, h-12, arc, arc);
            g2.setColor(Color.WHITE); g2.fillRoundRect(0, 0, w-12, h-12, arc, arc);
